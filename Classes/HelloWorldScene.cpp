...
void HelloWorld::onTouchesEnded(const std::vector<Touch*>& touches, Event  *event)
{
	...
	
	{
        ...
        request->setUrl("http://192.168.127.82/test.php?");
        request->setRequestType(HttpRequest::Type::POST);
		char * param = "name=okamoto&t=32&id=8";
...
//        curl_easy_setopt(curl, CURLOPT_URL, "http://192.168.127.82/test.php");
        curl_easy_setopt(curl, CURLOPT_URL, "http://192.168.127.82/test.php?name=nakamura&t=32&id=8");
...
void HelloWorld::onHttpRequestCompleted(HttpClient *sender, HttpResponse *response)
{
	...
	_label->setString( std::string(buffer->begin(), buffer->end() ) );
